...
**Answer:**
 ...
 High altitude allows the ball to fly further allowing for more home runs. Coors Field is in Denver, Colorado a high altitude city. It is possibe that the Coors Field, by virtue of its altitude tends to have more home runs per game.
...
# Count games in the dataset
print(f"\nTotal games in 2010: {len(df2010)}")
print(f"Total games in 2021: {len(df2021)}")
print(f"Combined total games: {len(df2010) + len(df2021)}")
```
...
**Answer:**
 ...
 There are 2430 games in 2010 and 2429 games in 2021. There are 4859 total combined games. Our goal is to predict whether or not Coors Field in Denver, Colorado is the most run-friendly baseball stadium in MLB. We need to compare stadium averages to determine this.
...
- **Categorical Data**: Scatterplots can often be used to show a particular linear trend. Bar charts can be used to group and rank categorical data.
- **Readability**: Stadium names are long text labels that fit better horizontally. With 30+ stadiums, horizontal bars prevent label overlap and rotation issues.
- **Comparison**: Bar charts make it easy to compare values across categories by comparing bar lengths, while scatter plots don't provide clear categorical boundaries.
- **Ordering**: We can easily sort stadiums by runs to show ranking.

**Vertical vs. Horizontal bar charts:**
- **Choose horizontal when**: You have many categories and/or long category labels.
- **Choose vertical when**: You have few categories or shorter labels
- **For our data**: Horizontal is better because we have 30 stadiums with 3-letter codes, and we want to emphasize the ranking by stadium. It would be harder to read the data labels with a vertical bar chart.
...
**Color highlighting in Approach 4:**
- **Focus**: Draws immediate attention to Colorado (COL) as the key finding
- **Comparison**: Makes it easy to see Colorado's position relative to other stadiums
- **Storytelling**: Reinforces the hypothesis that altitude affects performance

**Position vs. Color for encoding data:**
- **Position (x/y)**: Can rank by data point
- **Color**: Can draw immediate distinction to a particular data point
- **Best practice**: Use position for the main data (runs), color for emphasis (highlighting Colorado)
:::
...
# Add annotation for Colorado (REMOVED)
...
- There are a few things that make it suitable for a business presentation.
- **Labels**: Clearly labled X and Y axes, and clearly labeled presentation header. The categories which are the stadiums are also clearly labeled.
- **Obvious point**: With the title, the order ranking, and the single color, it is obvious what is being presented, that COL is the most run-friendly stadium.

- **Annotation**: The annotation which showed a purple left-pointing arrow and purple lettering indicating 'COL' looked awkward and unnecessary. COL is already distinct at the top and singularly colored as well as mentioned in the title. It has been removing by commenting the line that adds the annotation.
...
# Sort data separately for each year (highest to lowest, top to bottom)
...
# Create color arrays to highlight Colorado for each year
colors_2010 = ['#8B008B' if stadium == 'COL' else '#D3D3D3' for stadium in data_2010_sorted['home']]
colors_2021 = ['#8B008B' if stadium == 'COL' else '#D3D3D3' for stadium in data_2021_sorted['home']]

# 2010 visualization
bars_2010 = ax1.barh(data_2010_sorted['home'], data_2010_sorted['runs_2010'], color=colors_2010)
ax1.set_title('Average Runs Per Game by Stadium - 2010 Season', 
              fontsize=14, fontweight='bold', pad=15)
ax1.set_xlabel('Average Runs Per Game', fontsize=12, fontweight='bold')
ax1.set_ylabel('Stadium (Home Team)', fontsize=12, fontweight='bold')
ax1.grid(True, axis='x', alpha=0.3, linestyle='--')

# Add value labels on 2010 bars
for bar, value in zip(bars_2010, data_2010_sorted['runs_2010']):
    ax1.text(value + 0.05, bar.get_y() + bar.get_height()/2, 
             f'{value:.2f}', va='center', fontsize=9, fontweight='bold')

# 2021 visualization
bars_2021 = ax2.barh(data_2021_sorted['home'], data_2021_sorted['runs_2021'], color=colors_2021)
ax2.set_title('Average Runs Per Game by Stadium - 2021 Season', 
              fontsize=14, fontweight='bold', pad=15)
ax2.set_xlabel('Average Runs Per Game', fontsize=12, fontweight='bold')
ax2.set_ylabel('Stadium (Home Team)', fontsize=12, fontweight='bold')
ax2.grid(True, axis='x', alpha=0.3, linestyle='--')

# Add value labels on 2021 bars
for bar, value in zip(bars_2021, data_2021_sorted['runs_2021']):
    ax2.text(value + 0.05, bar.get_y() + bar.get_height()/2, 
             f'{value:.2f}', va='center', fontsize=9, fontweight='bold')

# Add legend
from matplotlib.patches import Patch
legend_elements = [Patch(facecolor='#8B008B', label='Colorado Rockies'),
                   Patch(facecolor='#D3D3D3', label='Other Stadiums')]
...
colorado_2010 = data_2010_sorted[data_2010_sorted['home'] == 'COL']['runs_2010'].iloc[0]
colorado_2021 = data_2021_sorted[data_2021_sorted['home'] == 'COL']['runs_2021'].iloc[0]

print(f"\n=== Colorado Performance Comparison ===")
print(f"2010: {colorado_2010:.2f} runs/game")
print(f"2021: {colorado_2021:.2f} runs/game")
print(f"Change: {colorado_2021 - colorado_2010:+.2f} runs/game")
print(f"Percentage change: {((colorado_2021 / colorado_2010) - 1) * 100:+.1f}%")

print(f"\n=== League Average Comparison ===")
print(f"2010 league average: {data_2010_sorted['runs_2010'].mean():.2f} runs/game")
print(f"2021 league average: {data_2021_sorted['runs_2021'].mean():.2f} runs/game")
print(f"League change: {data_2021_sorted['runs_2021'].mean() - data_2010_sorted['runs_2010'].mean():+.2f} runs/game")
```
...
This two-facet visualization effectively demonstrates the power of object-oriented matplotlib for comparative analysis. The visualization uses a side-by-side subplot layout (`fig, (ax1, ax2) = plt.subplots(1, 2)`) to create two independent horizontal bar charts that can be directly compared. Each chart is sorted independently by runs per game (ascending=True for horizontal bars), showing stadiums from most run-friendly (top) to least run-friendly (bottom) for their respective years.

The key insight from this visualization is that Colorado (highlighted in purple) consistently ranks among the highest-scoring stadiums in both 2010 and 2021, supporting the altitude hypothesis. The independent sorting reveals that while Colorado's absolute performance may change between years, its relative position as one of the most run-friendly stadiums remains consistent. This visualization successfully demonstrates all four stages of data visualization: clear purpose (comparing stadium effects across years), curated content (aggregated stadium averages), effective visual mapping (horizontal bars with color highlighting), and professional formatting (consistent styling, legends, and value labels).

The AI comes to the correct conclusion that Colorado is indeed one of the most run-friendly stadiums in both years, validating the hypothesis that high altitude creates a persistent advantage for offensive production at Coors Field.

**Answer:**

The AI comes to the correct conclusion that Colorado is one of the most run-friendly stadiums in both years, validating the hypothesis that high altitude creates a persistent advantage for offensive production at Coors Field. It did not hypothesize on why COL fell from the top spot in 2010 to the third from top spot in 2021. I do notice that COL did not see a decline in runs per game, but many stadiums did see an increase in runs per game. Additionally, while there were only three teams in 2010 that had averaged over 10 runs per game, there were five in 2021. There were eight teams in 2010 that averaged less than eight runs per game and there were only two in 2021 that averaged less than eight runs per game. This would require some further study to figure out why the runs per game across the league increased in 2021 relative to 2010.
:::
